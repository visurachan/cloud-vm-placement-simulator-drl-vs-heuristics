classdef WorstFitAgent < handle
    properties
        Config
    end

    methods
        function this = WorstFitAgent(config)
            this.Config = config;
        end
        ...
        function action = getAction(this, observation)
            ...
            best_server = 0;
            best_score = -1; % Higher score = more remaining capacity (worst fit wants maximum)
            
            % Check all servers and find the one with maximum remaining capacity after placement
            for server_id = 1:n_servers
                ...
                if (curr_cpu + cpu_needed <= 1.0) && (curr_mem + memory_needed <= 1.0) && ...
                   (curr_ssd + ssd_needed <= 1.0) && (curr_hdd + hdd_needed <= 1.0)
                    ...
                    % Calculate remaining capacity after placement (higher = worst fit preference)
                    ...
                    % Use sum of remaining resources as fitness score (higher = more leftover space)
            ...
            action = best_server; % Will be 0 if no server can host (reject)
        end
    end
end
