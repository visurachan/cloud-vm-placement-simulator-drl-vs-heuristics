function run_worstfit_allocation()
% RUN_WORSTFIT_ALLOCATION - Use Worst-Fit for VM allocation/evaluation.

    fprintf('=== Worst-Fit VM Allocation Deployment ===\n\n');
    ...
    % Save results to structured folders (without creating folders as you mentioned)
    timestamp = datestr(now, 'yyyy-mm-dd_HH-MM-SS');
    
    results_folder = fullfile('results', 'Worst_Fit');
    mat_folder = fullfile(results_folder, 'Matlab_format');
    txt_folder = fullfile(results_folder, 'Text_format');
    results_mat = fullfile(mat_folder, sprintf('worstfit_results_%s.mat', timestamp));
    save(results_mat, 'total_reward', 'step_count', 'placement_stats', ...
         'allocation_time_seconds', 'total_requests');

    results_txt = fullfile(txt_folder, sprintf('worstfit_results_%s.txt', timestamp));
    save_allocation_results_to_text(results_txt, step_count, allocation_time_seconds, ...
        total_reward, placement_stats, total_requests, 'Worst-Fit');
