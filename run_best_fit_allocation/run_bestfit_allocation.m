function run_bestfit_allocation()
% RUN_BESTFIT_ALLOCATION - Use Best-Fit for VM allocation/evaluation.

    fprintf('=== Best-Fit VM Allocation Deployment ===\n\n');
    ...
    % Create Best-Fit agent
    agent = create_bestfit_agent(config);
    ...
    % Save results to structured folders
    timestamp = datestr(now, 'yyyy-mm-dd_HH-MM-SS');
    
    % Define result folders for Best-Fit
    results_folder = fullfile('results', 'Best_fit');
    mat_folder = fullfile(results_folder, 'Matlab_format');
    txt_folder = fullfile(results_folder, 'Text_format');
   

    results_mat = fullfile(mat_folder, sprintf('bestfit_results_%s.mat', timestamp));
    save(results_mat, 'total_reward', 'step_count', 'placement_stats', ...
         'allocation_time_seconds', 'total_requests');

    results_txt = fullfile(txt_folder, sprintf('bestfit_results_%s.txt', timestamp));
    save_allocation_results_to_text(results_txt, step_count, allocation_time_seconds, ...
        total_reward, placement_stats, total_requests, 'Best-Fit');
